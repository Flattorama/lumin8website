...
export function PrismaticBurst({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WebGL not available, prismatic burst effect disabled');
      setWebglFailed(true);
      return;
    }
    ...
    try {
      ...
      console.warn('Failed to initialize WebGL shaders:', error);
      setWebglFailed(true);
      try {
    ...
    return (
      <div 
        style={{
          ...
          height: '100%',
          overflow: 'hidden',
          background: 'radial-gradient(ellipse at 30% 40%, rgba(120, 200, 191, 0.15) 0%, transparent 50%), radial-gradient(ellipse at 70% 60%, rgba(252, 223, 101, 0.12) 0%, transparent 50%), radial-gradient(ellipse at 50% 80%, rgba(242, 130, 181, 0.1) 0%, transparent 50%)'
        }}
      />
    );
  }
